...
public class GroupMeetingListAdapter extends RecyclerView.Adapter<GroupMeetingListAdapter.ViewHolder> {

    private List<GroupMeetingData> values;
    private List<GroupMeetingData> valuesCopy;
    ...
    public void notifyDataChange(List<GroupMeetingData> data) {
        Collections.sort(data, new Comparator<GroupMeetingData>() {
            public int compare(GroupMeetingData m1, GroupMeetingData m2) {
                try {
    ...
    public GroupMeetingListAdapter(Context context, List<GroupMeetingData> myDataset) {
    ...
    @Override
    public GroupMeetingListAdapter.ViewHolder onCreateViewHolder(ViewGroup parent,
                                                                 int viewType) {
            // create a new view
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        try {
            if (position != values.size()) {
                GroupMeetingData appointments = values.get(position);
                ...
                if (getMeetingStatus(appointments).equalsIgnoreCase("end")) {
                ...
                } else if (getMeetingStatus(appointments).equalsIgnoreCase("pending")) {
                ...
                } else if (getMeetingStatus(appointments).equalsIgnoreCase("cancel")) {
                ...
                if (appointments.getAppointmentType().equalsIgnoreCase("Fixed") &&
                        new Date().compareTo(meetingDateEndTime) <= 0
                        && getMeetingStatus(appointments).equalsIgnoreCase("pending")) {
                    holder.tv_confirm.setVisibility(View.VISIBLE);
                }else {
                    if (appointments.getAppointmentType().equalsIgnoreCase("Fixed"))
                        holder.tv_confirm.setVisibility(View.GONE);
                    else{
                        final Date flexibleMeetingDateEndTime = dateFormat.parse(appointments.getFdate()
                                + " " + appointments.getTotime());
                ...
                holder.tv_fname.setText("" + appointments.getGroup().getCreator1().getContact());
                holder.tv_email.setText("" + appointments.getGroup().getCreator1().getEmail());
                holder.tv_phone.setText("" + appointments.getGroup().getCreator1().getPhone());
    ...
    public interface OnAppointmentClickListener {
        void onAddToCalender(int position, GroupMeetingData data);

        void onOpenDetails(int position, GroupMeetingData data);

        void onCall(int position, GroupMeetingData data);

        void onNavigate(int position, GroupMeetingData data);

        void onConfirmMeeting(int position, GroupMeetingData data);

        void onCancelMeeting(int position, GroupMeetingData data);
    }
    ...
    public void filter(String text) {
        try {
            ...
            if (text.isEmpty()) {
                ...
                for (GroupMeetingData item : valuesCopy) {
